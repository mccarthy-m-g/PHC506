...
(A lot of core R functions fails these rules!)
...
```{r}
repeat_what_i_said <- function(what_i_said){
  sentence <- paste0("You said: '", what_i_said, "'")
  return(sentence)
}
...
starwars %>%
  ...
  mutate(
    bmi = calculate_bmi(height_in_cm = height, weight_in_kg = mass)
  )
```
...
starwars %>%
  ...
  mutate(
    bmi = calculate_bmi(height_in_cm = height, weight_in_kg = weight)
  )
...
starwars %>%
  select(name, height, weight = mass) %>%
  mutate(
    bmi = calculate_bmi(weight_in_kg = weight, height_in_cm = height)
  )
...
places, with a default of 2.

```{r}

calculate_bmi <- function(height_in_cm, weight_in_kg, n_decimals = 2){
  height_in_m <- height_in_cm/100
  bmi <- weight_in_kg / height_in_m ** 2
  round(bmi, digits = n_decimals)
}
...
We can update our calculate_bmi function to make use of this.
...
```
*Important*: when you pass arguments into a function via `...` the argument
names must match!
