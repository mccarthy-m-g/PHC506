...
Usually, this means taking "tidy" data and making it "untidy", but sometimes it
is helpful to have data in this format for displaying in tables or graphs.

Here is an example of a tidy data set. It describes traumatic brain injury rates
in the USA in 2014 as reported by the CDC. You can find more information about
this data set here:
https://github.com/rfordatascience/tidytuesday/tree/master/data/2020/2020-03-24

```{r}
tbi_age <- readr::read_csv(
  "https://raw.githubusercontent.com/rfordatascience/tidytuesday/master/data/2020/2020-03-24/tbi_age.csv"
)

tbi_age 
```

This data set has 231 rows, which is a lot for a table! What if we want to
display the number of incidents per age group with Injury Mechanism as a column?
...
Yourself - DRY code)! If you find yourself typing out a long list of things,
maybe there's a better way! (There's nearly always a better way!)
...
1. Using the traumatic brain injury data set (`tbi_age`), create a table 
of estimated rates per 100k people (`rate_est`) for hospitalizations, where
each row is an injury mechanism and each column is an age_group. 
...
3. Using the `nm_out` data frame described above, create a data frame where each
