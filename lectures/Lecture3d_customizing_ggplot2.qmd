...
usa_all_medals <- filter(medal_counts, NOC == "USA")
northern_hem <- filter(medal_counts_wide, NOC %in% c("USA", "CAN", "MEX", "CUB"))
```
...
There are many other ways we can customize plots to better communicate 
...
Sometimes you might want to add the results of a regression you did to a plot,
or indicate particular values on your plot. The family of functions
...
Customizing the way the non-data component of your plot will usually require 
...
`ggthemes` has everything you need to copycat the Economist plotting style: 
https://www.ggplot2-exts.org/ggthemes.html 

## Facets

Sometimes you might have too much data to easily display on one plot, and you
...
* `ggplot2` plots can be combined using the `grid.arrange()` function in the 
**`gridExtra`** package
...
Or you can use the **patchwork** package, which has a syntax similar to ggplot2,
and has nice features like collecting guides or axes across multiple plots:
...
different plot types based on the types of data they communicate, and comes 
...
results? Polish up your plot and then save it to the `figures` directory.
